...
import type { CheckInFormData, FormErrors, MoodType } from '@/types/wellness';
import type { WellnessEntry } from '@/types/wellness';

interface CheckInFormProps {
  onSubmit: (
    entry: Omit<WellnessEntry, 'id' | 'score'> & { id?: string },
  ) => WellnessEntry;
...
export function CheckInForm({ onSubmit, existingEntry }: CheckInFormProps) {
  ...
  if (submitted) {
    return (
      <Card className="text-center">
        ...
        <h3 className="mt-4 text-lg font-semibold text-gray-900">Check-In Saved!</h3>
    ...
    <form onSubmit={handleSubmit}>
      <Card className="space-y-6">
        ...
        <Button type="submit" size="lg" fullWidth>
          {existingEntry ? 'Update Today\'s Check-In' : 'Save Check-In'}
        </Button>
      </Card>
    </form>
  );
